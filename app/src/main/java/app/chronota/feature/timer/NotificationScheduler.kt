...
import app.chronota.MainActivity
...
import app.chronota.localizedContext
...
class NotificationScheduler(private val context: Context, private val repository: TimeRepository) {
    ...
    private suspend fun textContext(): Context {
        val application = context.applicationContext as? ChronotaApplication ?: return context
    ...
    private suspend fun taskName(title: String, categoryId: Long?): String {
        val preferences = (context.applicationContext as? ChronotaApplication)?.preferencesRepository?.preferences?.first()
    ...
    private fun alarmIntent(id: Long): PendingIntent = PendingIntent.getBroadcast(context, 0,
        Intent(context, AlarmReceiver::class.java).setAction(ALARM_ACTION).setData("chronota://alarm/$id".toUri()).putExtra("alarm_id", id),
    ...
    companion object {
        const val ALARM_ACTION = "app.chronota.ALARM"
