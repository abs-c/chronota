package app.chronotation.feature.category
...
val categoryPalette = app.chronotation.ui.theme.categoryColorRows.drop(1).map { it[3] }
        ...
        if (tab == 0) categoryIcons.chunked(5).forEach { row ->
            Row(Modifier.fillMaxWidth()) {
                row.forEach { (key, vector) -> Box(Modifier.weight(1f), contentAlignment = Alignment.Center) {
                    IconButton(onClick = { change(key, color) }, modifier = Modifier.size(app.chronotation.ui.theme.Metrics.controlHeight),
...
@Composable fun ColorPalette(color: Long, select: (Long) -> Unit) {
    app.chronotation.ui.theme.categoryColorRows.forEach { row ->
        Row(Modifier.fillMaxWidth()) {
