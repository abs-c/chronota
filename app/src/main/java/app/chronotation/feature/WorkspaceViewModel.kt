package app.chronotation.feature
...
import app.chronotation.data.repository.RuleViolation
import app.chronotation.domain.HistoricalPlanPolicy
...
class WorkspaceViewModel(application: Application) : AndroidViewModel(application) {
    val repository = (application as PlanRecordApplication).repository
    ...
    }.catch { error -> android.util.Log.e("plan-record", "Workspace observation failed", error); emit(WorkspaceState(loading = false, failed = true)) }
        ...
        viewModelScope.launch {
            while (true) {
                runCatching { repository.rolloverGoals(Instant.now(), ZoneId.systemDefault()) }
                    .onFailure { android.util.Log.e("plan-record", "Goal rollover failed", it) }
                delay(60_000)
            }
        }
    }
