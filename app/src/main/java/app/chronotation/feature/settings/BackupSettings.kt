...
import app.chronotation.data.backup.WebDavException
...
@Composable fun WebDavSettings(
    ...
    onBackupNow: (result: (Int?) -> Unit) -> Unit,
    onRestore: (result: (Int?) -> Unit) -> Unit,
) {
    ...
    EditorSheet(R.string.backup_webdav, onDismiss) {
        ...
        ActionButton(stringResource(R.string.backup_now), {
            state = BackupState.Busy
            onBackupNow { error -> state = if (error == null) BackupState.Idle else BackupState.Failed(error); if (error == null) message = R.string.backup_saved }
        ...
        ActionButton(stringResource(R.string.backup_restore), {
            state = BackupState.Busy
            onRestore { error -> state = if (error == null) BackupState.Idle else BackupState.Failed(error); if (error == null) message = R.string.backup_restored }
...
fun backupErrorResource(error: Throwable): Int = when (error) {
    ...
    is app.chronotation.data.backup.BackupFormat.Unreadable -> R.string.backup_error_format
