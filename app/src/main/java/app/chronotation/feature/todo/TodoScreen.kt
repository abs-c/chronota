package app.chronotation.feature.todo
...
import app.chronotation.feature.*
...
import app.chronotation.ui.components.EmptyState
import app.chronotation.ui.components.PageColumn
import app.chronotation.ui.components.PageHeader
...
fun TodoScreen(onSettings: () -> Unit, onCategories: () -> Unit, model: WorkspaceViewModel = workspaceModel(),
    ...
    val prefs by (androidx.compose.ui.platform.LocalContext.current.applicationContext as app.chronotation.PlanRecordApplication).preferencesRepository.preferences.collectAsStateWithLifecycle(app.chronotation.data.repository.AppPreferences())
    LaunchedEffect(model) { model.messages.collect { notice = it } }
    val now = rememberNow()
    val today = now.atZone(ZoneId.systemDefault()).toLocalDate()
    app.chronotation.feature.browse.BrowseScreen(false, state, { id -> planSeed = null; editing = id }, onOpenRecord,
