...
class BackupCoordinator(
    ...
    /** Pulls the rolling file back down and restores it. Throws [WebDavException] when it cannot. */
    suspend fun restoreNow(): BackupSummary {
        val current = preferences.preferences.first()
        if (!current.webDavConfigured) throw WebDavException(WebDavFailure.NOT_CONFIGURED)
        return backups.restore(backups.download(current.webDavTarget, current.webDavFolder))
    }
