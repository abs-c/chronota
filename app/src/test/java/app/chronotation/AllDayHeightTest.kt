...
class AllDayHeightTest {
    ...
    private fun state(): WorkspaceState {
        ...
        return WorkspaceState(loading = false,
            plans = listOf(
                Plan(id = 1, title = "All day", allDay = true, scheduledDate = LocalDate.now(), zoneId = ZoneId.systemDefault().id),
                ...
                Plan(id = 2, title = "Tiny", scheduledDate = LocalDate.now(), startTime = now, endTime = now.plusMinutes(1), zoneId = ZoneId.systemDefault().id),
                Plan(id = 3, title = "Tiny early", scheduledDate = LocalDate.now(), startTime = LocalTime.of(0, 5), endTime = LocalTime.of(0, 6), zoneId = ZoneId.systemDefault().id),
            ))
    }
