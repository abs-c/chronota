...
class AppNavigationTest {
    ...
    private val context get() = ApplicationProvider.getApplicationContext<PlanRecordApplication>()

    private fun launch() {
        compose.setContent {
            var mode by remember { mutableStateOf(ThemeMode.SYSTEM) }
            PlanRecordTheme(mode.isDark(false)) {
                PlanRecordApp(mode, { mode = it }, remember { SnackbarHostState() })
            }
        }
    }
