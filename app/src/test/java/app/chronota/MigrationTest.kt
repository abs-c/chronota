...
class MigrationTest {
    @Test fun versionOneUpgradePreservesRecordsPropertiesAndReminders() = runBlocking {
        ...
        val schema = JSONObject(File("schemas/app.chronota.data.db.AppDatabase/1.json").readText()).getJSONObject("database")
    ...
    @Test fun versionFourUpgradeAddsRecurrenceDefaultsWithoutTouchingRows() = runBlocking {
        ...
        context.deleteDatabase(name)
        val schema = JSONObject(File("schemas/app.chronota.data.db.AppDatabase/4.json").readText()).getJSONObject("database")
        context.openOrCreateDatabase(name, 0, null).use { old ->
            val entities = schema.getJSONArray("entities")
            for (i in 0 until entities.length()) {
                val entity = entities.getJSONObject(i)
    ...
    @Test fun versionFiveUpgradeAddsGoalsWithoutTouchingRows() = runBlocking {
        ...
        context.deleteDatabase(name)
        val schema = JSONObject(File("schemas/app.chronota.data.db.AppDatabase/5.json").readText()).getJSONObject("database")
        context.openOrCreateDatabase(name, 0, null).use { old ->
            val entities = schema.getJSONArray("entities")
            for (i in 0 until entities.length()) {
                val entity = entities.getJSONObject(i)
    ...
    @Test fun versionSixUpgradeAddsGoalNameAndPeriodValueWithoutTouchingRows() = runBlocking {
        ...
        context.deleteDatabase(name)
        val schema = JSONObject(File("schemas/app.chronota.data.db.AppDatabase/6.json").readText()).getJSONObject("database")
        context.openOrCreateDatabase(name, 0, null).use { old ->
            val entities = schema.getJSONArray("entities")
            for (i in 0 until entities.length()) {
                val entity = entities.getJSONObject(i)
    ...
    @Test fun versionSevenUpgradeAddsGoalStartHistoryAndPlanSkips() = runBlocking {
        ...
        context.deleteDatabase(name)
        val schema = JSONObject(File("schemas/app.chronota.data.db.AppDatabase/7.json").readText()).getJSONObject("database")
        context.openOrCreateDatabase(name, 0, null).use { old ->
            val entities = schema.getJSONArray("entities")
            for (i in 0 until entities.length()) {
                val entity = entities.getJSONObject(i)
    ...
    @Test fun everyShippedSchemaHasAMigration() = runBlocking {
        ...
        context.deleteDatabase(name)
        val schema = JSONObject(File("schemas/app.chronota.data.db.AppDatabase/9.json").readText()).getJSONObject("database")
        context.openOrCreateDatabase(name, 0, null).use { old ->
            val entities = schema.getJSONArray("entities")
            for (i in 0 until entities.length()) {
                val entity = entities.getJSONObject(i)
    ...
    @Test fun storedSecondsAreClearedOnUpgrade() = runBlocking {
        ...
        context.deleteDatabase(name)
        val schema = JSONObject(File("schemas/app.chronota.data.db.AppDatabase/10.json").readText()).getJSONObject("database")
        context.openOrCreateDatabase(name, 0, null).use { old ->
            val entities = schema.getJSONArray("entities")
            for (i in 0 until entities.length()) {
                val entity = entities.getJSONObject(i)
