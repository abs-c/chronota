...
- Match controls to their purpose: switches, plain inputs, radio/checkbox choices, chevrons for navigation, plus rows for adding, and centered colored save buttons. A choice that belongs to one control rather than to the page opens as a menu under that control — the calendar scale under its label, the statistics order under its sort button. Secondary pages have a back/title banner. Keep colors flat without gradients or heavy shadows.
- A page is either one canvas or a stack of cards, never both. A canvas — the timeline, the day and week calendar — paints `surface` and separates its bands with a hairline. A grouped page keeps the `background` gray and puts white `cardSurface` sheets on it: cards, row groups, fields, chips, the month grid's day list. Cards lift 1dp; a page's body sheet lifts 3dp so the gray fades under its edge instead of ending at a line; the date band a page's content slides under casts the same seam. What floats above everything (the glass dock, the action orb) uses 2dp, and a menu 4dp. Colors follow Apple's system values: `systemGroupedBackground` behind, `secondarySystemGroupedBackground` for the sheet, `systemGray5`/`4`/`2` for wells, quiet buttons and the segmented thumb, and the separator color for anything drawn as a line. Display-only lists omit the trailing chevron; keep it where the row navigates or edits.
